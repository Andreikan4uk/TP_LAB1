...
bool checkFilename(std::string file)
{
	if (file.size() < 4 && file[0] != '0') {
		std::cout << "Имя файла слишком короткое.\n";
		return false;
	}
						...
						&& ((file[4] == '.') || (file[4] == '\0'))))
	{
		...
		std::cout << "Данное имя файла используется Windows.\n";
		return false;
	}
	int i = file.size();
	...
	{
		if (file[i - 1] != '0')
			std::cout << "Файл должен иметь формат .txt\n";
		return false;
	}
}
...
bool InputInFileName(std::string& filename)
{
	...
	if (filename[0] != '0') {
		...
		std::cout << "Вы отказались от ввода данных из файла\n";
		return false;
	}
}
